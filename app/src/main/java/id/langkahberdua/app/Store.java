...
final class Store {
 ...
 synchronized void resolve(String id,boolean keepLocal){Row r=get(id);if(r==null||r.conflict==null)return;if(!keepLocal&&r.conflict.length()==0){db.delete("records","id=?",new String[]{id});return;}ContentValues cv=clearQueue();cv.put("accepted",r.conflict.toString());if(keepLocal){cv.put("pending",r.pending.toString());cv.put("base",r.conflict.optLong("version",0));cv.put("op",UUID.randomUUID().toString());}db.update("records",cv,"id=?",new String[]{id});}
